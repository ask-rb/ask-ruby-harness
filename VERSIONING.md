...
Patch numbers advance by exactly one per release: `0.3.11` → `0.3.12` →
`0.3.13`. Never skip or jump patch numbers (`0.3.11` → `0.3.13` is wrong),
...
gemchain guard ask-ruby-harness
gemchain update ask-ruby-harness <new-version> --dry-run
gemchain update ask-ruby-harness <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-ruby-harness.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/ruby/harness/version.rb` by gemchain, not by hand.
...
| `lib/ask/ruby/harness/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.3.11` → `0.3.12`.
- Several bug fixes ship together: one release, `0.3.11` → `0.3.12` (never
  `0.3.13`).
- New backwards-compatible API: `0.3.11` → `0.4.0`.
- Breaking change while pre-1.0: `0.4.0` → `0.5.0`, with a `Changed` /
  `Removed` changelog entry calling out the break.
- First stable release: `0.9.x` → `1.0.0`.
- This gem bumped because a dependency released: patch only, e.g. `0.3.12` →
  `0.3.13`, via the gemchain cascade — never hand-published.
